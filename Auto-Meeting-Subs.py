...
def whisper(file, output_loc, model, subformat, num_speakers, token, batch_size, dev):
    ...
    result = modload.transcribe(audio, batch_size=batch_size, print_progress=True,dev=dev)
    if dev:
    ...
    diarize_model = whisperx.DiarizationPipeline(use_auth_token=token, device=device, dev=dev)
    if dev:
